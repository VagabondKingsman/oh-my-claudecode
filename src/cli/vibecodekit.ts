...
function scaffoldCommand(args: readonly string[]): number {
  ...
  const layout: Array<{ dir: string; file: string; src: string }> = [
    {
      dir: '.omc/research',
  ...
  console.log(
    chalk.bold(
      `  /oh-my-claudecode:vibecodekit-hybrid --locale ${locale} "${slug}"`
    )
  );
  return 0;
}

function statusCommand(args: readonly string[]): number {
  const cwd = process.cwd();
  const deliverablesPath = resolve(cwd, '.omc/deliverables.json');
  if (!existsSync(deliverablesPath)) {
    console.error(
      chalk.yellow('No .omc/deliverables.json found. Run `omc vibecodekit scaffold <slug>` first.')
    );
    return 1;
  }
  let parsed: Record<string, unknown>;
  try {
    parsed = JSON.parse(readFileSync(deliverablesPath, 'utf8')) as Record<string, unknown>;
  } catch (err) {
  ...
  const gate = (parsed.verify_gate as string | null) ?? '—';
  const decision = (parsed.release_decision as string | null) ?? '—';
  const counts =
    (parsed.verdict_counts as Record<string, number> | null) ?? {
      pass: 0,
      fail: 0,
      painful: 0,
      missing: 0,
  ...
  console.log(
    `  verdict_counts   : pass=${counts.pass ?? 0} fail=${counts.fail ?? 0} painful=${counts.painful ?? 0} missing=${counts.missing ?? 0}`
  );
  ...
  if (updatedAt) console.log(`  updated_at       : ${updatedAt}`);
  return 0;
}
...
function localesCommand(): number {
  const pluginRoot = findPluginRoot();
  ...
  for (const e of entries) {
    if (e.isDirectory()) console.log(`  ${e.name}  (overlay at locale/${e.name}/)`);
  }
  return 0;
}

export async function vibecodekitCommand(rawArgs: readonly string[]): Promise<void> {
