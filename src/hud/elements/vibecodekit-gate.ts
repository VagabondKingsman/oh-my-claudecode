...
function decisionLabel(gate: VibecodekitGateForHud): string {
...
function decisionColor(gate: VibecodekitGateForHud): string {
  const glyph = gate.verifyGate;
  if (glyph === '🟢') return GREEN;
  if (glyph === '🟡') return YELLOW;
  if (glyph === '🔴') return RED;
 ...
 */
export function renderVibecodekitGate(gate: VibecodekitGateForHud | null): string | null {
  if (!gate) return null;
  ...
  const parts: string[] = [`${prefix}${color}VK:${label}${RESET}`];
 ...
 */
export function renderVibecodekitGateCompact(
  gate: VibecodekitGateForHud | null,
): string | null {
  if (!gate) return null;
  const glyph = gate.verifyGate;
  if (glyph) return `VK:${glyph}`;
  // Derive from release decision when glyph is missing.
  if (gate.releaseDecision === 'SHIP') return `VK:🟢`;
  if (gate.releaseDecision === 'SHIP_WITH_FOLLOWUPS') return `VK:🟡`;
  if (gate.releaseDecision === 'DO_NOT_SHIP') return `VK:🔴`;
  return `${DIM}VK:?${RESET}`;
}
