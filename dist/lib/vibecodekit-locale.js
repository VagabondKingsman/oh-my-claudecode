...
function normaliseLocale(raw) {
    ...
    return SUPPORTED_LOCALES.includes(head)
        ? head
        : null;
}
function readLocaleJson(directory) {
    try {
 ...
 */
export function resolveVibecodekitLocale(directory = process.cwd(), env = process.env) {
 ...
 */
export function getVibecodekitLocale(directory, env) {
    return resolveVibecodekitLocale(directory, env).locale;
}
//# sourceMappingURL=vibecodekit-locale.js.map