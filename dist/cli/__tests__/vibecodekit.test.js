...
describe('omc vibecodekit CLI', () => {
    ...
    it('scaffold creates canonical artifact skeleton + deliverables.json', () => {
        ...
        const deliverables = JSON.parse(readFileSync(join(workDir, '.omc/deliverables.json'), 'utf-8'));
        ...
        for (const relPath of expectedFiles) {
            ...
            expect(body, `${relPath} still contains an unresolved placeholder`).not.toMatch(/\{\{(?:slug|locale|date)\}\}/);
        }
        // And the slug must actually appear in the report headings (e.g.
        // "# RRI-T Report — checkout-flow") to confirm substitution worked.
        const rriT = readFileSync(join(workDir, '.omc/verify/vibecodekit-hybrid-rri-t-checkout-flow.md'), 'utf-8');
        ...
        const rri = readFileSync(join(workDir, '.omc/specs/vibecodekit-hybrid-rri-checkout-flow.md'), 'utf-8');
    ...
    it('scaffold slugifies noisy input', () => {
        ...
        expect(existsSync(join(workDir, '.omc/plans/vibecodekit-hybrid-my-checkout-flow-v2.md'))).toBe(true);
    });
...
//# sourceMappingURL=vibecodekit.test.js.map