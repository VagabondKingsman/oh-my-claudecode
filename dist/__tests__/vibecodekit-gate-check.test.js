...
function runScript(args, workDir, extraEnv = {}) {
...
describe('scripts/vibecodekit-gate-check.mjs', () => {
    ...
    it('honors VCK_DELIVERABLES_PATH env override', () => {
        ...
        writeFileSync(altFile, JSON.stringify({ verify_gate: '🟢', release_decision: 'SHIP' }));
        const r = runScript([], workDir, { VCK_DELIVERABLES_PATH: altFile });
        expect(r.exitCode).toBe(0);
        expect(r.summary).toContain('🟢 SHIP');
    });
    it('honors VCK_STRICT=1 env override', () => {
        writeFileSync(join(workDir, '.omc/deliverables.json'), JSON.stringify({ verify_gate: '🟡', release_decision: 'SHIP_WITH_FOLLOWUPS' }));
        const r = runScript([], workDir, { VCK_STRICT: '1' });
        expect(r.exitCode).toBe(1);
    });
});
//# sourceMappingURL=vibecodekit-gate-check.test.js.map